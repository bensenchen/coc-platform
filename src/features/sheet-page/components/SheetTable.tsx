...
export function SheetTable({ sheetPage, projectId }: Props) {
  ...
  const sortColumnId = (sheetPage.metadata.sortColumnId as string | undefined) ?? '';
  const sortDirection = (sheetPage.metadata.sortDirection as 'asc' | 'desc' | undefined) ?? 'asc';
  const filterText = (sheetPage.metadata.filterText as string | undefined) ?? '';
  ...
  const orderedRows = applyOrder(data?.rows ?? [], rowOrder);
  ...
  const rows = orderedRows
    ...
    .sort((a, b) => {
      ...
      const av = String(cells[a.id]?.[sortColumnId] ?? '');
      const bv = String(cells[b.id]?.[sortColumnId] ?? '');
      return (
        av.localeCompare(bv, undefined, { numeric: true }) * (sortDirection === 'asc' ? 1 : -1)
      );
    });
  ...
  function setAlign(align: 'left' | 'center' | 'right') {
    const coords = selectedCoords();
    const all =
      coords.length > 0 && coords.every(({ rowId, colId }) => fmtOf(rowId, colId).align === align);
    updateSelectionFmt((f) => {
      if (all) delete f.align;
      else f.align = align;
      return f;
    });
  }
  ...
  function cellMouseDown(r: number, c: number, e: React.MouseEvent) {
    const row = rows[r];
    const col = columns[c];
    if (editingCell && row && col && editingCell.rowId === row.id && editingCell.colId === col.id)
      return;
  ...
  function startColEdit(id: string, name: string) {
    if (columns.find((column) => column.id === id)?.pageId !== sheetPage.id) return;
    setColDraft(name);
    setEditingColId(id);
  }
  ...
  const allBold =
    selCoords.length > 0 && selCoords.every(({ rowId, colId }) => fmtOf(rowId, colId).bold);
  const allItalic =
    selCoords.length > 0 && selCoords.every(({ rowId, colId }) => fmtOf(rowId, colId).italic);
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex items-center gap-2 px-4 py-2 border-b border-slate-200 bg-white">
        <span className="text-sm font-medium text-slate-700">Management read model</span>
        <span className="rounded bg-amber-100 px-2 py-0.5 text-[10px] font-medium text-amber-800">
          Amber = local additions
        </span>
        ...
        <input
          aria-label="Filter management rows"
          value={filterText}
          onChange={(e) => updateFormat.mutate({ filterText: e.target.value })}
          placeholder="Filter rows…"
          className="w-32 rounded border border-slate-300 px-2 py-1 text-xs"
        />
        <select
          aria-label="Sort management rows"
          value={sortColumnId}
          onChange={(e) => updateFormat.mutate({ sortColumnId: e.target.value })}
          className="max-w-32 rounded border border-slate-300 px-2 py-1 text-xs"
        >
        ...
        <button
          onClick={() =>
            updateFormat.mutate({ sortDirection: sortDirection === 'asc' ? 'desc' : 'asc' })
          }
      ...
      {selRect && (
        <div className="flex items-center gap-2 px-4 py-1.5 border-b border-slate-200 bg-slate-50 text-xs select-none">
          ...
          <button onClick={toggleBold} className={fmtBtn(allBold)} title="Bold (Ctrl/Cmd+B)">
            <span className="font-bold">B</span>
          </button>
          <button onClick={toggleItalic} className={fmtBtn(allItalic)} title="Italic (Ctrl/Cmd+I)">
          ...
          <button
            ...
            className={fmtBtn(alignActive('left'))}
            title="Align left"
          >
            ⯇
          </button>
          <button
            ...
            className={fmtBtn(alignActive('center'))}
            title="Align center"
          >
            ≡
          </button>
          <button
            ...
            className={fmtBtn(alignActive('right'))}
            title="Align right"
          >
            ⯈
          </button>
          ...
          <button
            onClick={handleCopy}
            className="text-slate-600 hover:bg-slate-200 rounded px-1.5 py-0.5"
      ...
      <div className="flex-1 overflow-auto">
        {columns.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-slate-400 gap-2">
            <p className="text-sm">
              {linkedId
                ? 'The linked page has no columns yet.'
                : 'Standalone sheet — no columns yet.'}
            </p>
            <button
              onClick={() => setAddingCol(true)}
            ...
            <thead className="sticky top-0 z-10 bg-slate-50">
              <tr>
                ...
                {columns.map((col, cIdx) => {
                  ...
                  const headerBg = dragOver
                    ? '#eef2ff'
                    : colBg[col.id] || (own ? '#fde68a' : undefined);
                ...
                return (
                  ...
                  >
                    ...
                    {columns.map((col, cIdx) => {
                      ...
                      const bg =
                        f.bg || colBg[col.id] || rowBg[row.id] || (ownRow ? '#fde68a' : undefined);
                      return (
                        <td
                          key={col.id}
                          style={{
                          ...
                          className={`px-3 py-1.5 border-b border-r border-slate-100 select-none ${ownRow || isOwnCol(col.pageId) ? 'cursor-cell' : 'cursor-default text-slate-500'}`}
                          title={
                            ownRow || isOwnCol(col.pageId)
                              ? 'Local cell — double-click to edit'
                              : 'Inherited read-only value'
                          }
